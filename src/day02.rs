...
pub fn solve(contents: &String) -> std::io::Result<String> {
    let reports: Vec<Vec<u8>> = contents
        .lines()
        .map(|line| -> Vec<u8> {
            line.split_ascii_whitespace()
                .map(|level| -> u8 { level.parse::<u8>().expect("Could not parse level") })
                .collect()
        })
        .collect();
...
fn safe(report: &Vec<u8>) -> bool {
    let r: Vec<i8> = report
        .windows(2)
        .map(|window| window[0].checked_signed_diff(window[1]).unwrap())
        .collect();
    if (r.iter().all(|window| *window > 0) || r.iter().all(|window| *window < 0))
        && r.iter().all(|window| window.abs() <= 3)
    {
        true
    } else {
        false
...
fn part1(reports: &Vec<Vec<u8>>) -> u32 {
    reports
        .iter()
        .filter(|report: &&Vec<u8>| safe(*report))
        .count() as u32
}
fn part2(reports: &Vec<Vec<u8>>) -> u32 {
    reports
        .iter()
        .filter(|report: &&Vec<u8>| safe_with_dampener(*report))
        .count() as u32
}
