...
pub fn solve(contents: &String) -> std::io::Result<(i64, i64)> {
    let calibrations: Vec<Calibration> = contents
        .lines()
        .map(|line| {
...
fn get_operations(calibration: &Calibration) -> Option<Vec<Operation>> {
    ...
    let mut operations_combinations = itertools::repeat_n(
        [Operation::Add, Operation::Multiply],
        calibration.operands.len() - 1,
    )
    .multi_cartesian_product();
    let operation =
        operations_combinations.find(|operations| check_calibration(calibration, operations));
    operation
}
...
fn check_calibration(calibration: &Calibration, operations: &Vec<Operation>) -> bool {
    ...
    let result = operands
        .zip(operations.iter())
        .fold(first, |acc, (operand, operation)| match operation {
...
fn test_get_operations() {
    ...
    assert_eq!(
        operations,
        Some(vec![Operation::Add, Operation::Multiply, Operation::Add])
    );
}
